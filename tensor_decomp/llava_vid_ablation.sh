...
# ============================================================================
# llava_vid_ablation.sh  (DySeg-DPP — LLaVA-Video)
#
# 消融实验: 扫描核心参数 (DySeg + DPP + Top-K Soft Fusion)
#
# Pipeline:
#   SigLIP ViT (完全不动) → mm_projector → 2D Spatial Pool (average, stride=2)
#   → DySeg 分组 → 组预算分配 (Effective Rank / MCD)
#   → 组内 DPP anchor 选择 → Top-K 软分配 + 自适应垃圾桶融合
#   → LLM (Qwen2, 标准 1D RoPE) → (可选) 软剪枝 + 硬剪枝
#
# 可消融参数 (无 ViT 内部 token fusion, SigLIP 不支持):
...
# ############################################################################
# ██  模型配置 (LLaVA-Video specific)
# ############################################################################

PRETRAINED="/mnt/bn/zhibo-demo/huiqiang/zhibo_demo/LLM_models/LLaVA-Video-7B-Qwen2"
...
# ── DPP Anchor 选择方法 ──
BASE_ANCHOR_METHOD=dpp_optimized       # facility_location  facility_location_optimized   dpp  dpp_optimized
...
# ██  评测任务
# ############################################################################
# TASKS="videomme,longvideobench_val_v,egoschema_subset"   # mlvu_test  mlvu_dev  perceptiontest_test_mc  perceptiontest_val_mc
...
OUTPUT_DIR="./eval_results/llava_vid_ablation_study"
LOG_DIR="./eval_logs/llava_vid_ablation_study"
mkdir -p ${OUTPUT_DIR}
mkdir -p ${LOG_DIR}
...
run_experiment() {
    ...
    # MM_NEWLINE_POSITION 覆盖
    local P_MNP=${ABL_MM_NEWLINE_POSITION:-$MM_NEWLINE_POSITION}
...
EOF
    ...
    echo "  mm_newline_position:     ${P_MNP}"
    echo "  Output: ${RUN_DIR}"
    echo "============================================"
    ...
    for task in "${TASKS[@]}"; do
        ...
        accelerate launch --main_process_port 18888 \
            ...
            -m lmms_eval \
            --model llava_vid \
            --model_args pretrained=${PRETRAINED},attn_implementation=${ATTN_IMPLEMENTATION},conv_template=${CONV_TEMPLATE},max_frames_num=${MAX_NUM_FRAMES},video_fps=${VIDEO_FPS},mm_resampler_type=${MM_RESAMPLER_TYPE},mm_spatial_pool_stride=${MM_SPATIAL_POOL_STRIDE},mm_spatial_pool_out_channels=${MM_SPATIAL_POOL_OUT_CHANNELS},mm_spatial_pool_mode=${MM_SPATIAL_POOL_MODE},mm_resampler_location=${MM_RESAMPLER_LOCATION},mm_newline_position=${P_MNP},overwrite=${OVERWRITE},video_decode_backend=${VIDEO_DECODE_BACKEND},force_sample=${FORCE_SAMPLE},add_time_instruction=${ADD_TIME_INSTRUCTION},torch_dtype=${TORCH_DTYPE},enable_tensor_decomp=True,stage2_retention_ratio=${P_S2R},dyseg_threshold=${P_DST},cross_frame_lambda=${P_CFL},fusion_temperature=${P_FT},group_budget_method=${P_GBM},effective_rank_k=${P_ERK},w_dyn=${P_WDYN},asym_w=${P_AW},dynamism_window=${P_DWIN},cls_attn_method=${P_CAM},topk_fusion=${P_TK},trash_ratio=${P_TR},fusion_method=${P_FM},anchor_weight=${P_AWE},residual_ratio=${P_RR},query_prune_layer=${P_QPL},llm_prune_ratio=${P_LPR},llm_prune_method=${P_LPM},soft_prune_layer=${P_SPL},anchor_method=${P_AM},td_min_segment_num=${P_MSN},td_complementary_segment=${P_CS},res_imp_alpha=${P_RIA},res_method=${P_RM} \
            --tasks ${EVAL_TASK} \
            --batch_size 1 \
            "${LIMIT_ARGS[@]}" \
...
# echo "################################################################"
# for TK in 4 6 8 10 12 14 16; do
...
# echo "################################################################"
# for TR in 0.5; do
#     ABL_TRASH_RATIO=${TR} run_experiment "TR${TR}"
# done
...
# echo "################################################################"
# for AW in  0.65 0.7 0.75 0.8; do
...
# echo "################################################################"
# for RR in    0.2 0.25 0.3 0.35  0.4 0.45 0.5; do
...
# ============================================================================
# Ablation 21: MM_NEWLINE_POSITION 扫描
# ============================================================================
# echo ""
# echo "################################################################"
# echo "# Ablation 21: MM_NEWLINE_POSITION"
# echo "################################################################"
# for MNP in one_token frame no_token; do
#     ABL_MM_NEWLINE_POSITION=${MNP} run_experiment "MNP_${MNP}"
# done
...
echo ""
echo "================================================================"
echo " All LLaVA-Video ablation experiments complete!"
...
echo " Ablation 21 - MM_NEWLINE_POSITION:     one_token / frame / no_token"
echo "================================================================"
